(() => {
  ...
  function prepareTerminology(terms) {
    ...
    if (terminologyAliases.length) {
      terminologyPattern = new RegExp('(^|[^A-Za-z])(' + terminologyAliases.map(item => escapeRegExp(item.alias)).join('|') + ')(?=$|[^A-Za-z])','gi');
    }
  }
  ...
  function renderPlan() {
    ...
    blocksTarget.innerHTML = course.blocks.map((block,index) => {
      ...
      return `
        <details class="schedule-block" style="--block:${escapeHtml(block.colour)}">
          <summary>
            <span class="block-number">${index + 1}</span>
            <span class="block-summary-copy">
              <small>Lessons ${block.from}–${block.to}</small>
              <strong>${linkTerms(block.title)}</strong>
              <em>Suggested point: ${formatDate(targetDate)}</em>
            </span>
            <span class="block-toggle"><span class="open-word">Open</span><span class="close-word">Close</span> block <b>＋</b></span>
          </summary>
          <div class="block-body">
            <section>
              <h3>How the course develops here</h3>
              <p>${linkTerms(block.method)}</p>
            </section>
            <section>
              <h3>After this block, you should be able to</h3>
              <ul>${outcomes}</ul>
            </section>
            <p class="block-caution">These are practical learning aims, not an accredited level or a promise that every learner will progress at the same speed.</p>
          </div>
        </details>`;
    }).join('');
    ...
    if (scheduleStatus) {
      scheduleStatus.textContent = `${daysEachWeek} study ${daysEachWeek === 1 ? 'day' : 'days'} each week · ${course.sessionMinutes} minutes each day`;
    }
  }
  ...
  Promise.all([
    ...
    .then(([scheduleData,terminologyData]) => {
      course = scheduleData;
      prepareTerminology(terminologyData.terms || []);
      renderPlan();
    })
    .catch(error => {
      blocksTarget.innerHTML = `<div class="schedule-error"><strong>Schedule unavailable.</strong><br>${escapeHtml(error.message)}</div>`;
    });

  if (scheduleMethod && !scheduleMethod.querySelector('.terminology-guide-link')) {
    scheduleMethod.insertAdjacentHTML('beforeend',`<p class="terminology-note"><strong>New to grammar terminology?</strong> Highlighted words open a plain-English explanation. You can also <a class="terminology-guide-link" href="terminology.html?return=${terminologyReturn}">browse the complete terminology guide →</a></p>`);
  }

  const style = document.createElement('style');
  style.textContent = `
    .schedule-lede{max-width:850px;font-size:17px;line-height:1.7}
    .daily-target{display:grid;grid-template-columns:auto minmax(0,1fr);gap:16px;align-items:center;margin:20px 0 26px;padding:18px 20px;border:1px solid #e4d3a6;border-radius:17px;background:linear-gradient(135deg,#fff7d9,#fffdf5)}
    .daily-target .clock{display:grid;place-items:center;width:64px;height:64px;border-radius:50%;background:var(--gold);color:var(--ink);font:bold 20px Georgia,serif}
    .daily-target h2{margin:0 0 4px;font-size:23px}.daily-target p{margin:0;color:#655744;line-height:1.55}
    ...
    .schedule-method strong{color:var(--ink)}.terminology-note{margin:13px 0 0;padding-top:13px;border-top:1px solid #d6def1}.terminology-guide-link{color:var(--burgundy);font-weight:900;text-decoration-thickness:2px;text-underline-offset:3px}
    ...
    .schedule-block summary{display:grid;grid-template-columns:44px minmax(0,1fr) auto;gap:13px;align-items:center;padding:17px;cursor:pointer;list-style:none}.schedule-block summary::-webkit-details-marker{display:none}
    .block-number{display:grid;place-items:center;width:40px;height:40px;border-radius:50%;background:var(--block);color:#fff;font:bold 17px Georgia,serif}
    .block-summary-copy small,.block-summary-copy strong,.block-summary-copy em{display:block}.block-summary-copy small{color:var(--block);font-size:10px;font-weight:900;letter-spacing:.1em;text-transform:uppercase}.block-summary-copy strong{margin:3px 0;font:700 19px Georgia,serif}.block-summary-copy em{color:#746963;font-size:12px;font-style:normal}
    .block-toggle{padding:8px 10px;border-radius:9px;background:#f3eee9;color:#4c403a;font-size:11px;font-weight:900;white-space:nowrap}.block-toggle b{display:inline-block;margin-left:3px;font-size:16px;transition:transform .2s ease}.close-word{display:none}.schedule-block[open] .open-word{display:none}.schedule-block[open] .close-word{display:inline}.schedule-block[open] .block-toggle b{transform:rotate(45deg)}
    .block-body{display:grid;grid-template-columns:1fr 1fr;gap:18px;padding:0 20px 20px;border-top:1px solid #eee6e1;background:#fffdfb}.block-body section{padding-top:18px}.block-body h3{margin:0 0 9px;font-size:18px}.block-body p,.block-body li{color:#5e5550;line-height:1.6}.block-body ul{margin:0;padding-left:20px}.block-body li+li{margin-top:7px}.block-caution{grid-column:1/-1;margin:0!important;padding:12px 14px;border-radius:11px;background:#f7f3ef;font-size:12px!important}
    .term-link{color:var(--burgundy);font-weight:800;text-decoration:underline;text-decoration-style:dotted;text-decoration-thickness:2px;text-underline-offset:3px}.term-link:hover{text-decoration-style:solid}.term-link:focus-visible{outline:3px solid rgba(36,75,155,.28);outline-offset:2px;border-radius:3px}
    .schedule-error{padding:20px;border:1px solid #d8b4b4;border-radius:14px;background:#fff5f5;color:#7a2222}
    @media(max-width:900px){.schedule-builder{position:static}.block-body{grid-template-columns:1fr}}
    @media(max-width:620px){.daily-target{grid-template-columns:1fr}.schedule-block summary{grid-template-columns:40px minmax(0,1fr)}.block-toggle{grid-column:1/-1;text-align:center}}
  `;
